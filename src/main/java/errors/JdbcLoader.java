...
public class JdbcLoader {
	...
	public static final String INPUT_FILE = "C:\\Users\\jpalomo\\Desktop\\wm_data\\errors_output.txt";
	public static final String OUTPUT_DIR = "C:\\Users\\jpalomo\\Desktop\\wm_data\\resolved";
			...
			while (scanner.hasNextLine() && batch < BATCHES_TO_PROCESS) {
				...
				while (scanner.hasNextLine() && (line = scanner.nextLine()) != null && noOfBatchRecords < RECORDS_PER_BATCH) {
					...
					try {
						...
						builder.append(String.format("%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n", source.getBatchId().trim(),
								source.getVendorId().trim(), source.getProductId().trim(), source.getLabId().trim(),
								source.getState().trim(), source.getTestedAt().trim(), source.getExpiresAt().trim(),
				...
				try (PrintWriter printWriter = new PrintWriter(
						new File(String.format("%s\\resolved_output_%d.txt", OUTPUT_DIR, batch)))) {
